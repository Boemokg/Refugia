...
x = ggplot(Totalspp, aes(x = Model_Type, y = Estimate, fill = Covariates)) +
  ...
  geom_text(aes(label = asterisks), 
            ...
            vjust = ifelse(Totalspp$Estimate >= 0, 0.3, -0.2), 
            size = 20, angle = 90)  +
...
Y = ggplot(Totalspp.spt, aes(x = Model_Type, y = Estimate, fill = Covariates)) +
  ...
  geom_text(aes(label = asterisks), 
            position = position_dodge2(width = 1, preserve = "single"), 
            vjust = ifelse(Totalspp.spt$Estimate >= 0, 0.5, -0.5), 
            size = 20, angle = 90)  +
...
# AIC
aics_t <- AIC(tot_clim.sp,tot_prod.sp,tot_het.sp,tot_full.sp)

# Delta AIC
delta.aics <- aics_t$AIC - min(aics_t$AIC)

# Akaike weights
wi <- exp(-0.5*delta.aics)/sum(exp(-0.5*delta.aics))

# log Likelihoods
logliks <- c(logLik(tot_clim.sp),logLik(tot_prod.sp),logLik(tot_het.sp), logLik(tot_full.sp))

# model names
models <- c("Climatic Stability", "Productivity","Heterogeneity", "Full")                         

(modtable_t <- data.frame(models, -2*logliks, numpar=aics_t$df, AIC=aics_t$AIC, delta.aics,wi))
