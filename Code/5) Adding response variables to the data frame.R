...
Richness.FCTNRE.QDS <- readRDS("Data/Richness.FCTNRE.QDS") %>% st_set_geometry(NULL) %>% 
  rename("Facultative_species" = "species_richness") %>% dplyr:: select(-Description)
Richness.ObNRE.QDS <- readRDS("Data/Richness.ObNRE.QDS")%>% st_set_geometry(NULL) %>%
...
#Return all obligate wetland dependent narrow range endemics 
...
rm(bigdat,CapeFR_for_heatmap,Dom.geo, Heat.Mapdat, NREs, NRE_heatmap_Cloud, qds, Richness.FCTNRE.QDS, Richness.ObNRE.QDS, species_richness_QDS)

###################### Proceed to 7) Water and energy covariates
