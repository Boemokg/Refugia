...
##############################################################################
###############################################################################
##########################The best narrow range endemic model 
##############################################################################
...
NRE <- S.CARleroux(formula = CFR_data_anlys$Narrow_Range_endemics ~ 
...
plot(NRE$samples$beta)

# Species richness data cleaning 

#extract values from model 

Full.NRE <- NRE$summary.results %>% as.data.frame()
Full.NRE = Full.NRE[-1,]
Full.NRE = Full.NRE[-12,]
Full.NRE = Full.NRE[-12,]
Full.NRE$Covariates = c('Median CloudFQ', 'Wetland%', 'Mean CSI',
                    'MAT', 'MAP', 'PET', 'pH', 'P',
                    'Range in MAT', 'Roughness in MDQ','Roughness in PET')

#Totalspp = rbind(climate,productivity,Heterogeneity,Full)

Full.NRE$Model_Type = rep("Narrow-range endemics", 11)
...
# Reorder the factors 
Full.NRE$Covariates <- factor(Full.NRE$Covariates, levels = custom_order)


# change data frame name
Narrow_range_endemics = Full.NRE

# Save data frame 
saveRDS(Narrow_range_endemics, file = "Data/NRECARBAYES")


NRE_SPP <- ggplot(Full.NRE, aes(x = Covariates, y = Mean))+
  geom_pointrange(aes(ymin = `2.5%`, ymax = `97.5%`),data = Full.NRE,  size = 2)+
geom_hline(yintercept = 0, linetype = 2, size = 2)+
  theme(panel.background = element_rect(fill = "white"))+
 #scale_y_continuous(limits = c(-0.60, 0.60), breaks = seq(-0.60, 0.60, by = 0.1), 1)
  labs(title = "b)")+
  theme(panel.background = element_rect(fill = "white"),
...
# Save 
ggsave("Output/Bayesian models/Fig.2.png", NRE_SPP, width = 80, height = 50, units = "cm")


##############################################################################
###############################################################################
########################## Proceed to : 3 Wetland NRE modeling
