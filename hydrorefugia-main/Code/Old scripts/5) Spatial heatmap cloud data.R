...
species_richness_QDS <- species_richness_QDS %>% dplyr:: select(qdgc,QDS_richness)

#Leftjoin to only keep QDS richness 
Heat.Mapdat<- left_join(Heat.Mapdat, species_richness_QDS, by = c("Name","qdgc"))
...
ggplot()+
  geom_sf(data = Heat.Mapdat, aes(), lwd = 0)+
  geom_polygon()+
...
not_sure <- not_sure %>% 
  rename(Total_species_richness ="QDS_richness", Narrow_range_spp = "species_richness")

not_sure = not_sure %>% dplyr:: select(-pentad)

not_sure = not_sure %>% st_set_geometry(NULL)

CFR_data_fin = CFR_data_fin  %>% 
  dplyr::select(-Narrow_range_spp.x, -Range_Dry_rainfall.x, -Range_Temperature.x, -Range_Rainfall.x, -pentad.x)

CFR_data_fin = CFR_data_fin %>% rename(qdgc = "qdgc.x")

CFR_data_fin = left_join(CFR_data_fin, not_sure, by = "qdgc")


saveRDS(CFR_data_fin, "CFR_data_fin")


#Save Heat.Mapdat
saveRDS(Heat.Mapdat, file = "Data/Heat.Mapdat.RDS")

#removed saved objects 
rm(Dom.geo, Map_1, Map_12,Map_2, NREs, NREs_heatmap_cloud, 
   NREs, qds,RICHNESS.Cloud, RICHNESS.Cloud.QDS, CapeFR_for_heatmap, 
   Cloud.Maps, NRE_heatmap_Cloud, bigdat, not_sure, Heat.Mapdat, 
   species_richness_QDS, Richness.FCTNRE.QDS, Richness.ObNRE.QDS)

###################### Proceed to script Manuscript (Manuscript) 
