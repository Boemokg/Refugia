...
Annual_Mean_Temperature <- terra::extract(Annual_Mean_Temperature,CFR_data_fin)
...
CFR_data_fin$Range_Temperature <- temperature_range$temperature_range
...
#intersect the raster with the QDS layer , so extract them using the QDS shapefile
Annual_Rainfall <- terra:: extract(Annual_Rainfall,CFR_data_fin)
...
#add the data the co-variate to the QDS data as "Annual_Mean_Temperature"
CFR_data_fin$Range_Rainfall <- rainfall_range$rainfall_range
...
#intersect the raster with the QDS layer , so extract them using the QDS shapefile
Dry_Rainfall <- terra:: extract(Dry_Rainfall,CFR_data_fin)
...
#add the data the co-variate to the QDS data as "Annual_Mean_Temperature"
CFR_data_fin$Range_Dry_rainfall <- Dry_rainfall_range$Dry_rainfall_range
...
PET_Range <- terra:: extract(PET_Range, CFR_data_fin)
...
PET_Range<- aggregate(sum ~ ID, data = PET_Range, mean)

#add the values to the QDS data
CFR_data_fin$PET_Range <- PET_Range$sum
...
Roughness_Temperature <- terra::terrain(Annual_Mean_Temperature, "roughness")

#extract values rainfall values according to the capefr data 
Roughness_Temperature <- terra:: extract(Roughness_Temperature, Heat.mapdat)
...
Roughness_Temperature<- aggregate(roughness ~ ID, data = Roughness_Temperature, mean)

#add the values to the qds data
Heat.mapdat$Roughness_Temperature <- Roughness_Temperature$roughness
...
Roughness_Rainfall <- terra::terrain(Annual_Rainfall, "roughness")

#extract values rainfall values according to the QDS data 
Roughness_Rainfall <- terra:: extract(Roughness_Rainfall, Heat.mapdat)
...
Roughness_Rainfall <- aggregate(roughness ~ ID, data = Roughness_Rainfall, mean)

#add the values to the QDS data, Annual_Rainfall will show up as 
Heat.mapdat$Roughness_Rainfall <- Roughness_Rainfall$roughness
...
#Determining the elevation values of the raster
Roughness_Dry_Rainfall <- terra::terrain(Dry_Rainfall, "roughness")

#extract values rainfall values according to the QDS data 
Roughness_Dry_Rainfall <- terra:: extract(Roughness_Dry_Rainfall, Heat.mapdat)

#Removing Na values in the covariate 
Roughness_Dry_Rainfall$roughness[is.na(Roughness_Dry_Rainfall$roughness)] <- 0

#Aggregate the IDs so that there is one average Dry_Rainfall per QDS
Roughness_Dry_Rainfall<- aggregate(roughness ~ ID, data = Roughness_Dry_Rainfall, mean)

#add the values to the QDS data
Heat.mapdat$Roughness_Dry_Rainfall <- Roughness_Dry_Rainfall$roughness
...
#Determining the elevation values of the raster
PET_Roughness <- terra::terrain(raster_stack, "roughness")

#extract values rainfall values according to the QDS data 
PET_Roughness <- terra:: extract(PET_Roughness, Heat.mapdat)

#Removing Na values in the covariate 
PET_Roughness$roughness[is.na(PET_Roughness$roughness)] <- 0

#Aggregate the IDs so that there is one average Dry_Rainfall per QDS
PET_Roughness<- aggregate(roughness ~ ID, data = PET_Roughness, mean)

#add the values to the QDS data
Heat.mapdat$PET_Roughness <- PET_Roughness$roughness
...
Soil_ph <- terra:: extract(Soil_ph,Heat.mapdat)
...
#add the values to the QDS data
Heat.mapdat$Mean_Soil_ph <- Mean_Soil_ph$pH

#add the values to the QDS data
Heat.mapdat$Median_Soil_ph <- Median_Soil_ph$pH


#read the P data
Ext_P <-rast("/Users/NHLKAG001/Desktop/Data/Ext_P_mg_kg.tif")
#extract the mean PH values and Median PH values 
Ext_P <- terra:: extract(Ext_P,Heat.mapdat)
...
#add the values to the QDS data
Heat.mapdat$Mean_Ext_P <- Mean_Ext_P$p

#add the values to the QDS data
Heat.mapdat$Median_Ext_P <- Median_Ext_P$p

#saveRDS(unique_qds, file = "Data/unique_qds")
#saveRDS(CFR_data, file = "Data/CFR_data.RDS")

rm(Annual_Mean_Temperature, Annual_Rainfall, Clim.var, Dry_rainfall_range, Dry_Rainfall, Ext_P, 
   Mean_Ext_P, Mean_PET, Mean_Soil_ph, PET, PET_Roughness,rainfall_range, raster, raster_stack, 
   Roughness_Rainfall, Roughness_Dry_Rainfall, Roughness_Temperature, Soil_ph, temperature_range, 
   Median_Ext_P, Median_PET, Median_Soil_ph)
